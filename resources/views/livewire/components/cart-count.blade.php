         ...
         @else
             @if ($cartCount->count())
                 @foreach ($cartCount as $cart)
                     <li class="cart-item">
                         <div class="item-img">
                             <a href="{{ route('product-detail', $cart->product->id) }}"><img
                                     src="/products/{{ $cart->product->image }}" alt="Commodo Blown Lamp"></a>
                        ...
                        <span class="rating-number">(64)</span>
                             ...
                             <h3 class="item-title"><a
                                     href="{{ route('product-detail', $cart->product) }}">{{ $cart->product->name }}</a>
                             ...
                             <div class="pro-qty item-quantity">
                                 ...
                                 <input wire:change="update({{ $cart->id }}, $event.target.value)" type="number"
                                     class="quantity-input" value="{{ $cart->quantity }}" disabled>
